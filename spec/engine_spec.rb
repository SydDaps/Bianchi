...
RSpec.describe Bianchi::USSD::Engine do
  ...
  let(:session) do
    session = Bianchi::USSD::Session.new correct_params
    session.menu = menu
  ...
  context "Operation" do
    it "renders am initial menu pages request" do
      ...
      expect(engine_object.prompt_data).to eq({"activity_state"=>:await, "body"=>"test", "mobile_number"=>"+233557711911", "session_id"=>"345344322123"})
    end
  end
end
